...
function get_all_gejala($pdo) {
    if (!$pdo) {
        ...
        return [
            ...
            ['id_gejala' => 'G28', 'purpose' => 'Produksi telur menurun'], // Fix: used purpose instead of nama in previous write? No, previous was nama.
...
function get_riwayat($pdo, $search = '', $id_penyakit = '') {
    if (!$pdo) {
        ...
        if ($id_penyakit) {
            $mock_data = array_filter($mock_data, function($r) use ($id_penyakit) {
                return strpos($r['id_penyakit'], $id_penyakit) !== false;
            });
        }
    ...
    // Addressing code review feedback: join logic failed for multiple disease IDs
    // We'll fetch all diseases first as a map
    $disease_map = [];
    foreach(get_all_penyakit($pdo) as $p) {
        $disease_map[$p['id_penyakit']] = $p['nama'];
    }
    ...
    foreach ($history as &$row) {
        ...
        $names = [];
        foreach ($ids as $id) {
            if (isset($disease_map[$id])) $names[] = $disease_map[$id];
        }
        $row['nama_penyakit'] = !empty($names) ? implode(', ', $names) : 'Tidak Terdeteksi';
    }
 ...
 */
function increment_visitor($pdo) {
    if (!$pdo) return;
    $pdo->query("UPDATE pengunjung SET hits = hits + 1 WHERE id_pengunjung = 1");
}

function get_visitor_count($pdo) {
    if (!$pdo) return 0;
    return $pdo->query("SELECT hits FROM pengunjung WHERE id_pengunjung = 1")->fetchColumn();
}
?>
